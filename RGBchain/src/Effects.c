...
//  }
  ...
  switch (rnd)
  {
    ...
    case  4: Eff_Candle_2(c_yellow, EFF_DURATION_3MIN); break;
    case  5: Eff_Rainbow(EFF_DURATION_3MIN);
...
{
  const RGB_colors_e arrColors[] = { c_red, c_green, c_blue, c_white_dark, c_violet, c_yellow };
  uint32_t g_nColorIndex = 0;
  ...
  for (uint8_t i = 0; i < g_nLeds; i++)
  {
    ...
    g_nColorIndex++;
    if (g_nColorIndex >= sizeof(arrColors) / sizeof (RGB_colors_e))
    {
      g_nColorIndex = 0;
    }

    RGB_colors_e eColor = arrColors[g_nColorIndex];
